...
import type { ParsedCiValueMap } from '../validation/initialValues';
import { extractAllCiValues } from '../validation/initialValues';
...
export interface DiagramAnalysis {
  ...
  /** Parsed `name → value` map of every Initial Conditions block in the
   *  diagram. Used by VariablesPanel and the CI label mirror to flag
   *  literal mismatches against the Variables table's `initialValue`. */
  ciValues: ParsedCiValueMap;
  /** Subset of `ciValues` keys whose CI literal disagrees with the
   *  variable's `initialValue` declared in the Variables panel.
   *  Pre-computed so the label mirror can paint individual CI lines red
   *  without re-running the comparison per render. */
  divergentInitialNames: Set<string>;
}
...
export function analyzeDiagram(
  ...
  // CI-block snapshot. Hand the same `nodes` array we already walked.
  const ciValues = extractAllCiValues(nodes as Parameters<typeof extractAllCiValues>[0]);

  // Pre-compute divergent names so per-line painting in the CI label
  // mirror is cheap. Strict rule: a name is divergent iff the variable
  // exists in the panel AND the CI assigns it a scalar literal AND
  // either the panel value is missing or differs. `data` kind is exempt
  // (its panel input is disabled by convention). Array-shaped seeds are
  // also exempt since CI lines are line-scalar.
  const divergentInitialNames = new Set<string>();
  for (const v of variables as readonly {
    name: string;
    kind: string;
    initialValue?: unknown;
  }[]) {
    const ci = ciValues.get(v.name);
    if (!ci || ci.kind !== 'literal') continue;
    if (Array.isArray(v.initialValue)) continue;
    if (v.kind === 'data') continue;
    if (v.initialValue === undefined || v.initialValue !== ci.value) {
      divergentInitialNames.add(v.name);
    }
  }

  return { undefinedRefs, unusedVars, dataVarNames, ciValues, divergentInitialNames };
}
