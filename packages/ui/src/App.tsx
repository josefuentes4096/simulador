...
export function App() {
  ...
  // Canonical JSON of the last-saved (or last-loaded) model. Used to gate
  // the "discard unsaved changes?" prompt — if the current serialization
  // matches this, nothing has been edited since open/save and the
  // confirmation is skipped. null while a brand-new untitled file is open
  // and untouched.
    ...
    async (knownPath?: string) => {
      ...
      try {
        ...
        if (opened) {
          ...
          // Pin the just-loaded canonical JSON as the dirty-tracking baseline.
          // Closing/quitting after Open with no edits won't trigger the
          // "discard?" confirmation.
          setSavedSerialized(JSON.stringify(opened.model));
  ...
  const onSave = useCallback(async () => {
    ...
    try {
      ...
      if (result.path) {
        ...
        // Re-baseline dirty-tracking against the freshly-written content.
        // We mutate `builtWith` on `serialized` above; reflect that in the
        // baseline so the very next dirty-check after save sees no diff.
        setSavedSerialized(JSON.stringify(serialized));
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
    }
  ...
  // Asks the user to confirm only when the current model differs from the
  // last-saved/loaded snapshot — opening a file and immediately closing
  ...
  const blankAndReset = useCallback(
    (confirmLabel: string) => {
      ...
      // Real dirty-check: compare the current canonical JSON against the
      // baseline pinned by the last Open/Save. When the baseline is null
      // (brand-new untitled file), fall back to the simple "has anything
      // been added?" heuristic so the prompt still fires for a hand-built
      // model the user hasn't saved yet.
      const isDirty =
        savedSerialized !== null
          ? JSON.stringify(model.serialize()) !== savedSerialized
          : hasContent;
      if (
        isDirty &&
    ...
    [history, model, savedSerialized, t],
  );
    ...
    const unsubscribe = window.simulador.onMenuAction((action) => {
      ...
      switch (action.type) {
        ...
        case 'check-for-updates':
          // Mark this as a manual check so the UpdateBanner shows the
          // "you're up to date" toast — the silent startup check
          // suppresses it.
          (window as unknown as { __manualUpdateCheck?: boolean }).__manualUpdateCheck = true;
          void window.simulador.checkForUpdates();
          break;
      }
    });
    return unsubscribe;
