...
function LabelInput({
  ...
  label: string;
  placeholder: string;
  // When true (CI block label), each `IDENT = VALUE` line is checked
  // against the Variables panel's `initialValue` and painted red on
  // divergence.
  isCi?: boolean;
}) {
  ...
  // For CI blocks: extract the LHS identifier from the line and check the
  // pre-computed divergent set so render is O(1) per line.
  const lineDivergent = (line: string): boolean => {
    if (!isCi) return false;
    const m = /^\s*([\p{L}_][\p{L}\p{N}_]*)\s*=/u.exec(line);
    if (!m) return false;
    return analysis.divergentInitialNames.has(m[1]!);
  };
  return (
    <span className="node-label__wrap">
      ...
      >
        {lines.map((line, lineIdx) => (
          <span
            key={lineIdx}
            className={`node-label__mirror-line ${
              !showPlaceholder && lineDivergent(line) ? 'node-label__mirror-undef' : ''
            }`}
          >
            {line === ''
              ? ' '
...
export function InitialConditionsNode({ id, data, selected }: NodeProps<FlowNodeT>) {
  return (
    <div className={`node node--initial ${selected ? 'node--selected' : ''}`}>
      <LabelInput id={id} label={data.label ?? ''} placeholder="C.I." isCi />
      <Handle type="source" position={Position.Bottom} />
    </div>
  );
}
