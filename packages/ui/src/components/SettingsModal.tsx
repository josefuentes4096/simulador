...
export function SettingsModal({ open, onClose, onResolutionsChanged }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const auto = await window.simulador.getAutoCheckUpdates();
      if (cancelled) return;
      setUrl(stored);
      setOriginalUrl(stored);
      setState(cached);
      setError(cached.errorMessage);
      setAutoCheckUpdatesState(auto);
    })();
    return () => {
      cancelled = true;
    };
  ...
  return (
    <Modal open={open} onClose={onClose} title={t('settings.title')}>
      ...
      <section className="settings-section">
        <h3>{t('settingsUpdate.header')}</h3>
        <p className="settings-hint">{t('settingsUpdate.hint')}</p>
        <div className="settings-row">
          <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <input
              ...
              onChange={async (e) => {
                ...
                try {
                  await window.simulador.setAutoCheckUpdates(next);
                } catch {
            ...
            />
            <span>{t('settingsUpdate.autoCheck')}</span>
          </label>
        </div>
      </section>
    </Modal>
