...
import { extractAllCiValues, variableInitialDiverges } from './initialValues';
...
export function validate(model: SimulationModel): ValidationIssue[] {
  ...
  // Cross-check Variables panel `initialValue` against the Initial
  // Conditions block(s). When the variable is declared in both AND the
  // CI assigns it a literal, the panel must declare the same literal —
  // an empty panel value also counts as a mismatch. The user has to
  // pick one source of truth.
  const ciValues = extractAllCiValues(model.diagram.nodes);
  for (const v of model.behavior.variables) {
    if (variableInitialDiverges(v, ciValues)) {
      const ci = ciValues.get(v.name);
      if (ci && ci.kind === 'literal') {
        const panelLabel =
          v.initialValue === undefined ? t('validation.initialValueEmpty') : String(v.initialValue);
        issues.push({
          severity: 'error',
          message: t('validation.initialValueDiverges', {
            name: v.name,
            panel: panelLabel,
            ci: String(ci.value),
          }),
