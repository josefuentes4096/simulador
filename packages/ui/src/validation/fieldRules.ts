...
import {
  variableInitialDiverges as _variableInitialDiverges,
  ciLineDiverges as _ciLineDiverges,
  type ParsedCiValue,
} from './initialValues';
...
export const fieldRules = {
   ...
   * declared in the Variables panel. Always returns false — variables can
   * legitimately live only in the diagram (the cátedra's classification of
   * variables is loose enough that users may want to omit some). The
   * `analysis.undefinedRefs` set is still computed for diagnostics; this
   * predicate just no longer paints them red.
   */
  diagramRefUndefined(_identifier: string, _analysis: DiagramAnalysis): boolean {
    return false;
  },

  /**
   * The variable's panel `initialValue` disagrees with what the Initial
   * Conditions block declares for the same identifier. Renders red so the
   * user can pick one.
   */
  variableInitialDiverges(
    variable: ModelVariable,
    ciValues: Map<string, ParsedCiValue>,
  ): boolean {
    return _variableInitialDiverges(variable, ciValues);
  },

  /**
   * Inside an Initial Conditions block label, this specific `name = value`
   * line disagrees with the variable's panel `initialValue`. Used by the
   * label mirror to paint just the offending line red.
   */
  ciLineDiverges(
    name: string,
    ci: ParsedCiValue,
    ...
    return _ciLineDiverges(name, ci, [...variables]);
  },
