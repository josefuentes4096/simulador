...
function createWindow(): void {
  ...
  // lives at packages/app/build/. In packaged builds electron-builder copies
  // the icon dir into <resources>/build/ (extraResources mapping). On Windows
  // the taskbar wants a multi-resolution .ico — passing .png there often
  // renders a blurry or default icon when the window is minimized.
...
function registerIpc(): void {
  ...
  // Auto-update IPC. The fire-and-forget calls (`Check`, `Download`,
  // `Install`) don't return progress — listeners get that via the
  // broadcast `UpdaterStatus` channel. `GetStatus` lets a freshly-mounted
  // renderer fetch the current state instead of waiting for the next event.
  ...
  ipcMain.handle(IPC.UpdaterInstall, async (): Promise<void> => {
    quitAndInstall();
  });
  ipcMain.handle(IPC.UpdaterGetStatus, () => getLastUpdateStatus());
  ipcMain.handle(IPC.SettingsGetAutoCheckUpdates, (): Promise<boolean> =>
    getAutoCheckUpdates(),
  );
  ipcMain.handle(IPC.SettingsSetAutoCheckUpdates, (_evt, value: boolean): Promise<void> =>
    ...
    { label: 'Window', role: 'windowMenu' },
    {
      ...
      submenu: [
        ...
        { type: 'separator' },
        {
          ...
          click: () => sendMenuAction({ type: 'check-for-updates' }),
        },
      ],
    },
  ];
...
app.whenReady().then(async () => {
  // Without this, Windows can't associate the running window with the
  // installed app's identity, so the taskbar falls back to a generic
  // electron.exe icon. Must match `appId` in electron-builder.yml.
  if (process.platform === 'win32') app.setAppUserModelId('com.simulador.flow');
  registerIpc();
  ...
  // Kick the auto-update check a few seconds after the window is up — but
  // only when the user hasn't opted out via Preferences. The manual
  // "Help → Check for updates" item still works regardless.
  // Silently fails when running in dev mode (`!app.isPackaged`).
